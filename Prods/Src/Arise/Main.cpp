...
float g_pfAspects[] = // Considering a viewport with 2.0 aspect
{
	2.0f, 1.875f, 2.66666f, 2.4f, 3.0f, -1.0f
};
...
public:
	...
	bool CheckRequirements()
	{
		...
		{
			CLogger::ErrorWindow("No ARB_vertex_program extension found");
			return false;
		}
	...
	void OnLoading(float fPercentage)
	{
		...
		if(g_bLowQuality)
		{
			glViewport(0, 75, 800, 400);
		}
		else
		{
			glViewport(0, 128, 1024, 512);
		}
		...
		float fX      = 0.736328125f;
		float fY      = 0.750000000f;
		float fWidth  = 0.168945312f;
		float fHeight = 0.007812500f;

		CVector4 v4BarCol(0.0f, 0.388235294f, 0.65098039f, 1.0f);

		UtilGL::Rendering::DrawRect(NULL,
									fX, fY, fWidth * fPercentage * 0.01f, fHeight,
									0.0f, v4BarCol, UtilGL::States::BLEND_ONE, UtilGL::States::BLEND_ZERO);
